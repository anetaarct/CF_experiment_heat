...
dir.create("models/sensitivity_temperature_interaction", recursive = TRUE, showWarnings = FALSE)
...
extract_temperature_interaction <- function(fit, response, response_label, model_name) {
  coef_tab <- summary(fit)$coefficients$cond
  term <- "incubation_temperature_sc:nestling_temperature_sc"
...
fit_temperature_interaction <- function(data, response) {
  ...
  saveRDS(fits, file.path("models/sensitivity_temperature_interaction", paste0("temperature_interaction_models_", response, ".rds")))
  write_csv(comparison, file.path("models/sensitivity_temperature_interaction", paste0("temperature_interaction_comparison_", response, ".csv")))

  for (nm in names(fits)) {
    capture.output(
      summary(fits[[nm]]),
      file = file.path("models/sensitivity_temperature_interaction", paste0("summary_", response, "_", nm, ".txt"))
    )
  }

  extract_temperature_interaction(fits$interaction_gaussian, response, response_label, "interaction_gaussian")
}

interaction_terms <- lapply(later_responses, function(response) {
  fit_temperature_interaction(dat, response)
})
...
saveRDS(
  ...
  "models/sensitivity_temperature_interaction/temperature_interaction_models_D12_SURV.rds"
)
write_csv(surv_comparison, "models/sensitivity_temperature_interaction/temperature_interaction_comparison_D12_SURV.csv")
capture.output(summary(surv_additive), file = "models/sensitivity_temperature_interaction/summary_D12_SURV_additive_binomial.txt")
capture.output(summary(surv_interaction), file = "models/sensitivity_temperature_interaction/summary_D12_SURV_interaction_binomial.txt")

interaction_terms <- bind_rows(
  interaction_terms,
  extract_temperature_interaction(surv_interaction, "D12_SURV", response_labels[["D12_SURV"]], "interaction_binomial")
)

write_csv(interaction_terms, "models/sensitivity_temperature_interaction/temperature_interaction_terms.csv")

message("Temperature-interaction sensitivity models finished. Outputs saved in models/sensitivity_temperature_interaction/.")